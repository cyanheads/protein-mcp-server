...
|:-----|:------------|:-----------|:------------|
| `protein_search_structures` | Search experimental + predicted structures by free text, sequence, or organism/method/resolution filters. Routes to RCSB Search v2, which indexes both the experimental PDB and computed models (`results_content_type`: `experimental` / `computational` / both) — predicted structures search through the same surface. Returns ranked, metadata-enriched hits — chain identifiers into `protein_get_structure`. Sequence hits (experimental and computed-model alike) retain the matched polymer as `entityId` while exposing the chainable entry as `id`. Optionally returns a **facet breakdown** (counts by method / organism / release year) alongside the hits for instant corpus orientation at no extra upstream call. | `query` (text), `sequence` (1-letter AA, triggers mmseqs2), `organism`, `method`, `max_resolution`, `content_type` (`experimental`/`predicted`/`all`), `facets` (optional dimensions to summarize), `start`, `limit` | `readOnlyHint`, `openWorldHint` |
| `protein_get_structure` | Fetch structures with metadata and coordinate-file URLs. `source` selects experimental (PDB ID), predicted (UniProt accession), or `best_available` (federated pick via 3D-Beacons). Surfaces confidence (pLDDT / PAE) on predicted models. Accepts up to 25 IDs in one batched RCSB GraphQL call with **per-ID partial success** — resolved structures return alongside a `failed[]` list naming any IDs that missed and why, and `requested` / `processed` disclose any IDs dropped by the batch cap. Entry-endpoint records also carry polymer entities (both `authAsymIds` and `labelAsymIds`), ligands, molecular weight, and release date. Large mmCIF returns an outline-on-overflow section index — a lone over-budget file is withheld with a pointer to its `coordinateUrls`. `coordinateUrls` lists only files that exist: BinaryCIF from `models.rcsb.org`, no PDB format when the entry reports `pdb_format_compatible: N`, and a computed model's files from its provider (all three formats from the AlphaFold DB prediction record, best-effort — a failed lookup keeps the RCSB BinaryCIF and says so in `notice`; mmCIF from ModelArchive by entry ID). | `ids` (1–25 PDB IDs or UniProt accessions), `source`, `include_coords` (bool), `sections` (outline re-call) | `readOnlyHint`, `openWorldHint` |
| `protein_find_similar` | Find structurally or evolutionarily related proteins. `by: sequence` → RCSB mmseqs2 sequence search; `by: structure` → Foldseek async search against PDB + AlphaFold/BFVD/ESM databases. Output names the engine and database it answered from. Both modes report `totalCount`/`start`/`nextStart`; a completed structure job returns its `ticketId`, so re-calling with `ticket_id` and a new `start` pages the same finished job instead of resubmitting the structure. Foldseek splits a multichain structure into one query per chain: a structure response covers one query (`query`, default 0) and reports `queryCount`; its hits are ranked by `score` across all searched databases before paging. | `by` (`sequence`/`structure`), `pdb_id` or `uniprot` or raw `sequence`, `databases`, `query`, `max_evalue`, `min_identity`, `start`, `limit` | `readOnlyHint`, `openWorldHint` |
| `protein_track_ligands` | Ligand discovery and binding-site analysis. `mode: find_ligand` resolves a name/formula to chemical component IDs — a formula-shaped query routes to the Search API's `chemical`/`formula` terminal for an exact-composition match, anything else to the chemical-dictionary name/synonym search; candidates are re-ranked by deposition frequency within a pool of `max(limit, 25)`, and `totalCount` (RCSB's match total) beside `candidatesConsidered` (the pool) discloses — with a narrow-the-query `notice` — when the pool truncated the match set; `mode: structures_with_ligand` returns PDB entries bound to a given ligand (exact `comp_id`); `mode: binding_site` returns the **interacting protein residues** lining a ligand's pocket in a given structure (residue, position, chain, contact distance — via RCSB `rcsb_target_neighbors`), each in both the mmCIF label namespace (`asymId`/`seqId`) and the author namespace (`authAsymId`/`authSeqId`), plus the ligand instance's author chain and residue number. Returns chemical metadata (formula, weight, SMILES) on resolution. | `mode`, `query` (name/formula) or `comp_id`, `pdb_id` (for `binding_site`), `start` (`structures_with_ligand`, `binding_site` instances), `limit` | `readOnlyHint`, `openWorldHint` |
...
|:-------------|:------------|:-----------|
| `pdb://{entry_id}` | Experimental structure summary (title, method, resolution, organism, ligands, and per-entity chain IDs in both the author and mmCIF label namespaces) — injectable context twin of `protein_get_structure` for `source: experimental`. | none (single entity) |
...
|:--------|:------|:--------|
| `rcsb-service` | RCSB **Search API v2** (`search.rcsb.org/rcsbsearch/v2/query`, POST — incl. faceted aggregation), **Data API** REST + **GraphQL** (`data.rcsb.org` — incl. `rcsb_target_neighbors`), coordinate-file URLs (`files.rcsb.org` mmCIF/PDB, `models.rcsb.org` BinaryCIF) | `search_structures`, `get_structure`, `find_similar` (sequence), `track_ligands`, `analyze_collection` |
...
| `RCSB_FILES_BASE_URL` | No | Override the RCSB mmCIF / PDB-format download base (default `https://files.rcsb.org`). |
| `RCSB_MODELS_BASE_URL` | No | Override the RCSB BinaryCIF (ModelServer) base (default `https://models.rcsb.org`). |
...
| `MODELARCHIVE_BASE_URL` | No | Override the ModelArchive download base (default `https://modelarchive.org`). |
...
| poll | `GET /api/v1/structures/results?uuid=<uuid>` per job | `404 "No such UUID"` (or a `200` with `info.status: RUNNING` and no `results`) = not yet ready → shared `withAsyncPoll` backs off until `PROTEIN_ASYNC_POLL_TIMEOUT_MS`. A completed job echoes its submitted pair in `results[0].structures[]` and its method in `meta.alignment_method`; a resumed pair's row takes `a`/`b` from that echo, since `modeledResidues`, `coverage`, and the TM-score (normalized by the first structure's length) are all ordered as submitted |
...
| # | Call | Purpose |
|:--|:-----|:--------|
| 0 | resolve query to a coordinate file | PDB ID → `files.rcsb.org/download/{id}.cif` (mmCIF exists for every entry, PDB format does not; Foldseek reads both); UniProt → AlphaFold `pdbUrl` |
...
| 3 | `GET /api/result/queries/{id}/{limit}/0` | List the job's queries — one per chain of the submitted file, in file order (`job_A`, `job_B`, …). Gives `queryCount` and range-checks `query`: step 4 answers an out-of-range index with HTTP 200 and empty blocks, so the check can't be left to the provider. Read in one oversized request, since the endpoint's `limit`/`offset` paging is off by one upstream. |
| 4 | `GET /api/result/{id}/{query}` | Fetch one query's alignments: per-database blocks, each sorted by `score` descending; per-target `seqId`, `alnLength`, `prob`, `eval`, `score`, `qAln`/`dbAln`; target headers like `AF-P01541-F1`. AlphaFold DB targets follow `AF-{UniProtAccession}-F{fragment}` — extract the accession (`P01541`) to chain into `protein_get_structure` or `protein_get_annotations`. PDB targets arrive as assembly files (`1crn-assembly1.cif.gz_A`); the leading four characters are the entry ID. The blocks are flattened and re-sorted by `score` descending (missing last, ties by database then target) before `start`/`limit` slicing — `score` is the one field comparable across databases. |
...
| `find_similar` | `query_out_of_range` | `InvalidParams` | `query` is not below the completed job's `queryCount`. Foldseek itself answers such an index with HTTP 200 and no hits, which would read as a search with no matches. | Re-call with the returned `ticket_id` and a `query` from 0 to `queryCount - 1`; the job is complete, so nothing is resubmitted. |
| `find_similar` | `mode_mismatched_field` | `InvalidParams` | An input field only the other `by` mode consumes was supplied — `sequence`/`max_evalue`/`min_identity` under `by: structure`, or `ticket_id`/`query`/a non-empty `databases` under `by: sequence`. Each branch reads only its own subset, so the field would be dropped silently and an unfiltered or unscoped search would look filtered. An empty `databases` array matches the handler's own fallback and is not a violation; `start` and `limit` are shared by both modes. Guarded handler-side rather than by a Zod refine, so the rejection carries `data.reason` and the declared recovery hint. | Drop the field, or switch `by` to the mode that reads it — `sequence`/`max_evalue`/`min_identity` apply to `by: sequence`, `ticket_id`/`databases`/`query` to `by: structure`. |
...
| `analyze_collection` | `duplicate_dimension` | `InvalidParams` | `group_by` repeats a dimension, which would nest a facet inside itself upstream — same-attribute overlap, not a cross-tab. | List each dimension at most once: one dimension for a breakdown, or two distinct dimensions for a cross-tab. |
| `analyze_collection` | `interval_not_applicable` | `InvalidParams` | `interval` was supplied but neither requested `group_by` dimension aggregates by a histogram accepting that value's type — a numeric width needs `resolution`/`molecular_weight`, `"year"` needs `release_year`. Sending it anyway would succeed with default bins and read as a filtered result. | Group by an interval-capable dimension, or drop `interval` and let each dimension use its own default. |
| `analyze_collection` | `zero_match` | (success, empty) | `total: 0` with `buckets: []` per requested dimension. Disclosed via a `ctx.enrich` notice keyed by `content_type`, not an error, and it takes precedence over the predicted-content caveat so a 0 total is not misattributed to the content universe. | Broaden or drop the `query`/`organism`/`method`/`max_resolution` filters, or widen `content_type` when a narrower universe was selected. |
| `search_structures` | `duplicate_dimension` | `InvalidParams` | `facets` repeats a dimension; RCSB collapses the request to one raw facet and both specs map back onto it, returning the breakdown twice. | List each facet dimension at most once; drop the repeated value and re-call. |
| `search_structures` | `sequence_modifier_without_sequence` | `InvalidParams` | `min_identity` or `max_evalue` was supplied with no `sequence`. `buildQuery()` reads both only inside its sequence branch, so the threshold never reaches RCSB and an unfiltered search looks filtered. Guarded handler-side rather than by a Zod refine, so the rejection carries `data.reason` and the declared recovery hint. | Add a `sequence` to run a similarity search these thresholds can filter, or drop `min_identity`/`max_evalue`. |
| `compare_structures` | `no_distinct_pair` | `InvalidParams` | Every entry in `structures[]` denotes the same structure, so no pair remains once repeats are collapsed. | Pass at least two different structures (entry ID, or entry ID + chain); a structure repeated in the list is compared once. |
| `compare_structures` | `resume_method_mismatch` | `InvalidParams` | A resumed job's echoed `meta.alignment_method` differs from the call's `method`. Unlike orientation, a method can't be remapped, so reporting the call's method over the job's scores would mislabel them. Checked only once the job completes; thrown after the fanout settles so no sibling pair keeps polling. | Re-call with the method the job ran, or drop that resume entry to submit a fresh alignment. |
| `compare_structures` | `resume_job_mismatch` | `InvalidParams` | A resume entry's `uuid` belongs to a job whose echoed pair differs from its `a`/`b` labels. | Copy each entry's `uuid` from the same `pairs[]` row as its `a` and `b`, or drop the entry. |
...
- **Ligand full-text is noisy.** `service: full_text` for a ligand name returns loosely-matched components (querying "imatinib" surfaced amino acids). `track_ligands` uses `service: text_chem` with `comp_id` `exact_match` for structures-with-ligand, and routes name→comp_id resolution through the chemical-dictionary search, not free text. Formula queries take a different terminal entirely — `service: chemical` with `parameters.type: formula` — because the `text_chem` attribute `chem_comp.formula` is not searchable upstream (`400 "search is not enabled on [ chem_comp.formula ] attribute"`), and a formula run through the name search returns components whose *names* contain formula-like tokens.
...
- **Pocket residues need both numbering namespaces.** `rcsb_target_neighbors` reports the mmCIF label chain and position (`target_asym_id`, `target_seq_id`) plus the author residue number (`target_auth_seq_id`), and no author chain. The label and author numbering are related by no fixed offset (1IEP label THR93 is author THR315), so `binding_site` returns both, taking `authSeqId` verbatim. The author chain comes from the entry's per-instance `polymer_entity_instances.rcsb_polymer_entity_instance_container_identifiers { asym_id auth_asym_id }` pairs in the same GraphQL call — not the entity-level `asym_ids`/`auth_asym_ids` arrays, which RCSB sorts independently (6QNR_9: `["I","OB"]` vs `["82","8E"]`, where label I is author 8E), so zipping them mislabels chains. Insertion codes are out of reach: no binding-neighbor or chain-identifier object in the Data API carries one.
- **Ligand-name candidate pools can truncate.** `find_ligand` re-ranks by deposition frequency within `max(limit, 25)` candidates. Specific names fit ("ATP": 7 matches), broad words do not ("iron": 119, "acid": 12,412). The name search offers no continuation that would make paging meaningful, so the tool reports `totalCount` against `candidatesConsidered` and tells the reader to narrow the query rather than exposing an offset.
- **Facet cardinality is capped.** Terms facets can be large (organism alone exceeds 1,000 buckets). `analyze_collection` caps buckets per dimension level (`bucket_limit`, defaulting to `PROTEIN_FACET_BUCKET_CAP`, 50) and discloses truncation as a `notice` fragment per capped position — the top-level dimension and each nested cross-tab child, since a cross-tab caps every position independently — alongside a coarse `truncated` flag. `search_structures` applies the same cap to its flat facets, with no per-call override, and its `notice` names each capped dimension and routes to `analyze_collection` with a larger `bucket_limit`; a sequence search is told to narrow the request instead, since `analyze_collection` cannot reproduce one.
...
| RCSB GraphQL | `POST data.rcsb.org/graphql` | `entries(entry_ids:[...])` batches N entries in one call (title, exptl method, resolution, MW) — the N+1 killer. `nonpolymer_entity_instances.rcsb_target_neighbors` → binding-site residues `{target_asym_id, target_comp_id, target_seq_id, target_auth_seq_id, distance}`; the instance's own `rcsb_nonpolymer_entity_instance_container_identifiers` carries `auth_asym_id` and `auth_seq_id` (a string, e.g. `"201"`). Verified: 1IEP (imatinib/STI) → 23 contacts (ILE138, ASP159, THR93 … with Å distances). Field is `rcsb_target_neighbors`, **not** `rcsb_ligand_neighbors` (undefined). |
