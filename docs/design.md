...
| `beacons-service` | **3D-Beacons** federated API (`www.ebi.ac.uk/pdbe/pdbe-kb/3dbeacons/api`) | `search_structures` (predicted), `get_structure` (`best_available`), `get_annotations` (PDB→UniProt resolve) |
...
Single-pair request body (verified): `{"context":{"mode":"pairwise","method":{"name":"tm-align","parameters":{}},"structures":[{"entry_id":"4HHB","selection":{"asym_id":"A"}},{"entry_id":"2HHB","selection":{"asym_id":"A"}}]}}`.
...
**`protein_get_structure` (batch + best_available)** is multi-call but synchronous: one RCSB GraphQL `entries()` call for N experimental IDs (no N+1), or per-accession 3D-Beacons/AlphaFold for predicted/best-available, fanned out with `Promise.allSettled` and a concurrency cap; unresolved IDs collect into `failed[]`.
